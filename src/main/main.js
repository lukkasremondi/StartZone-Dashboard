...
// ADIÇÃO: Handler para buscar detalhes de um jogo específico para a nova tela.
ipcMain.handle('get-game-details', async (event, gameName) => {
    try {
        // Delega a busca para a função que criamos no games-info.js
        const details = await gamesInfo.getGameDetails(gameName);
        return details;
    } catch (error) {
        console.error('Erro no handler get-game-details:', error);
        return null; // Retorna null em caso de erro para o frontend tratar
    }
});
...
ipcMain.handle('launch-game', async (event, game) => {
    try {
        ...
        // Passa o objeto 'game' e a 'mainWindow' para a função de launch
